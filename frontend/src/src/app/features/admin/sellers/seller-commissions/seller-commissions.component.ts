...
import { SellerWithdrawService } from 'src/app/core/services/seller-withdraw.service';
...
export class SellerCommissionsComponent implements OnInit {
  ...
  load(): void {
    ...
    this.financeService.getWallet(this.sellerId).subscribe({
      next: (w: any) => { this.wallet = w; this.availableBalance = Number(w?.availableBalance) || this.availableBalance; }
    });

    this.financeService.getTransactions(this.sellerId).subscribe({
      next: (rows: any[]) => this.transactions = rows || []
    });
  }
  ...
  submitWithdraw(): void {
    ...
    if (!this.withdrawForm.amount || this.withdrawForm.amount < 500) {
      this.withdrawError = 'Minimum withdrawal amount is ৳500.'; return;
    }
  ...
  statusClass(status: string): string {
    ...
    if (s === 'ON_HOLD') return 'badge-blue';
    if (s === 'REVERSED') return 'badge-red';
