...
export class AddProductComponent implements OnInit {
  ...
  product = {
    name: '',
    nameBn: '', unit: 'piece', weight: null as number | null, minimumOrderQuantity: 1,
    productionDate: '', expiryDate: '', organic: false, returnAvailable: true, deliveryAreas: '',
    ...
    if (id) {
      ...
      this.productService.getProductById(+id).subscribe({
        next: (p: any) => {
          this.product = {
            name: p.name ?? '',
            nameBn: p.nameBn ?? '', unit: p.unit ?? 'piece', weight: p.weight ?? null, minimumOrderQuantity: p.minimumOrderQuantity ?? 1,
            productionDate: p.productionDate ?? '', expiryDate: p.expiryDate ?? '', organic: p.organic ?? false, returnAvailable: p.returnAvailable ?? true, deliveryAreas: p.deliveryAreas ?? '',
            price: p.price ?? null,
  ...
  onSubmit(): void {
    ...
    formData.append('nameBn', this.product.nameBn);
    formData.append('unit', this.product.unit);
    formData.append('minimumOrderQuantity', String(this.product.minimumOrderQuantity || 1));
    formData.append('organic', String(this.product.organic));
    formData.append('returnAvailable', String(this.product.returnAvailable));
    formData.append('deliveryAreas', this.product.deliveryAreas);
    if (this.product.weight) formData.append('weight', String(this.product.weight));
    if (this.product.productionDate) formData.append('productionDate', this.product.productionDate);
    if (this.product.expiryDate) formData.append('expiryDate', this.product.expiryDate);
  ...
  resetForm(): void {
    this.product = {
      name: '', nameBn: '', unit: 'piece', weight: null, minimumOrderQuantity: 1, productionDate: '', expiryDate: '', organic: false, returnAvailable: true, deliveryAreas: '', price: null, salePrice: null, stock: null,
