...
export class AuthService {
    ...
    return this.http.post<AuthResponse>(`${this.apiUrl}/login`, data).pipe(
      tap(res => {
        if (res.success && res.user) {
          ...
          localStorage.setItem('rural_mart_user', JSON.stringify(normalized));
          if (res.token) localStorage.setItem('rural_mart_token', res.token);
  ...
  isLoggedIn(): boolean {
    return !!localStorage.getItem('rural_mart_user');
  }
  ...
  getToken(): string | null {
    return localStorage.getItem('rural_mart_token');
  }
  ...
  private getSavedUser(): any {
    const raw = localStorage.getItem('rural_mart_user');
